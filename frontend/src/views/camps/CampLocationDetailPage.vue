...
import type { CampDetailResponse, CreateCampRequest, UpdateCampRequest } from '@/types/camp'
...
const { loading, error, getCampById, updateCamp, refreshGooglePlaces } = useCamps()
const { fetchPhotos } = useCampPhotos()
...
onMounted(async () => {
  ...
  if (result) {
    camp.value = result
    // Load user-uploaded photos
    const photos = await fetchPhotos(campId)
    campPhotos.value = photos
  }
})
...
const handleSubmitEdit = async (data: CreateCampRequest | UpdateCampRequest) => {
  if (!camp.value) return

  const result = await updateCamp(camp.value.id, { ...data, isActive: (data as UpdateCampRequest).isActive ?? camp.value.isActive })
  if (result) {
    // Re-fetch full detail to refresh displayed data
    const refreshed = await getCampById(camp.value.id)
    if (refreshed) {
      camp.value = refreshed
    }
...
const handleRefreshGooglePlaces = async () => {
  if (!camp.value) return
  refreshing.value = true
  const result = await refreshGooglePlaces(camp.value.id)
  if (result) {
    camp.value = result
    toast.add({
      severity: 'success',
      summary: 'Éxito',
      detail: 'Datos de Google Places actualizados correctamente',
      life: 3000
    })
  } else {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: error.value || 'Error al actualizar datos de Google Places',
      life: 5000
    })
  }
...
<template>
  <div class="container mx-auto p-4">
    ...
    <!-- Loading State -->
    <div v-if="loading && !camp" class="flex justify-center p-12">
      <ProgressSpinner />
    </div>

    <!-- Error State -->
    <Message v-else-if="error && !camp" severity="error" :closable="false">
      {{ error }}
    ...
    <div v-else-if="camp">
      ...
      <!-- Inline Edit Mode -->
      <div v-if="isEditing" class="rounded-lg border border-gray-200 bg-white p-6">
        <CampLocationForm
          mode="edit"
          :camp="camp"
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div class="space-y-6">
            ...
            <CampContactInfo :camp="camp">
              <template #actions>
                <Button
                  v-if="camp.googlePlaceId && auth.isBoard"
                  label="Actualizar datos de Google"
                  icon="pi pi-refresh"
                  outlined
                  size="small"
                  :loading="refreshing"
                  data-testid="refresh-google-places-btn"
                  @click="handleRefreshGooglePlaces"
                />
              </template>
            </CampContactInfo>

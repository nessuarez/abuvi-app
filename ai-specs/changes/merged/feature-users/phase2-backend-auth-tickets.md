...
- [x] BCrypt.Net-Next package installed (version 4.0.*)
- [x] IPasswordHasher interface created with HashPassword and VerifyPassword methods
- [x] PasswordHasher class implements IPasswordHasher using BCrypt with work factor 12
- [x] All unit tests pass (TDD approach)
...
- [x] JWT settings added to appsettings.json (Issuer, Audience, ExpiryInHours)
- [x] JWT secret configured in user-secrets (NOT in appsettings)
- [x] Microsoft.AspNetCore.Authentication.JwtBearer package installed (version 9.0.*)
- [x] Configuration validation on startup (fails fast if secret missing)
...
1. Initialize and set user secret:
...
- [x] JwtTokenService class created with GenerateToken method
- [x] Token includes claims: sub (userId), email, role, jti
- [x] Token signed with HMACSHA256
- [x] Token expiry configurable via appsettings
- [x] All unit tests pass (TDD approach)
- [x] Service registered in DI container
...
- [x] JWT Bearer authentication registered in DI
- [x] Token validation parameters configured (ValidateIssuer, ValidateAudience, ValidateLifetime, ValidateIssuerSigningKey)
- [x] UseAuthentication() middleware added BEFORE UseAuthorization()
- [x] Authorization services registered
- [x] Startup fails fast with clear error if JWT secret missing
...
1. Add middleware after UseCors():
...
- [x] LoginRequest record created (Email, Password)
- [x] RegisterRequest record created (Email, Password, FirstName, LastName, Phone?)
- [x] LoginResponse record created (Token, UserInfo)
- [x] UserInfo record created (Id, Email, FirstName, LastName, Role)
- [x] LoginRequestValidator created with email and password validation
- [x] RegisterRequestValidator created with strong password requirements
...
- [x] AuthService class created with LoginAsync and RegisterAsync methods
- [x] LoginAsync validates credentials, checks IsActive, generates JWT token
- [x] RegisterAsync checks for duplicate email, hashes password, creates user with Member role
- [x] LoginAsync returns null for invalid credentials (not throwing exceptions)
- [x] RegisterAsync throws InvalidOperationException for duplicate email
- [x] All unit tests pass (TDD approach)
- [x] Service registered in DI container
...
1. Update Program.cs to inject IPasswordHasher into UsersService (already done if registered in DI)
2. Verify all tests pass: `dotnet test --filter "UsersServiceTests"`
...
1. **Production**: Create migration script to rehash existing passwords (not recommended - require password reset instead)

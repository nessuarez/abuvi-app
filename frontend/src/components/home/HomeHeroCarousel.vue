...
<template>
  <section aria-label="Novedades y accesos destacados" class="relative min-h-[60vh] overflow-hidden md:min-h-[70vh]">
    <!-- Background carousel -->
    <div class="absolute inset-0 z-0">
      <Galleria v-model:activeIndex="activeIndex" :value="slides" :num-visible="1" :auto-play="true"
        :transition-interval="10000" :circular="true" :show-thumbnails="false" :show-item-navigators="false"
        :show-indicators="false" class="h-full w-full">
        <template #item="{ item }">
          <div class="h-[60vh] w-full md:h-[70vh]">
            <img :src="item.image" :alt="item.imageAlt" class="h-full w-full object-cover" />
          </div>
        </template>
      </Galleria>
    </div>
    ...
    <!-- Gradient overlay -->
    <div class="absolute inset-0 z-10 bg-gradient-to-br from-yellow-900/80 via-amber-800/65 to-yellow-700/60" />
    ...
    <div
      ...
      <div
        ...
        <h1 class="mb-4 text-3xl font-bold text-white drop-shadow-lg md:text-5xl">
          {{ activeSlide.headline }}
        </h1>

        <p class="mb-8 text-lg font-medium text-amber-100 drop-shadow-md md:text-xl">
          {{ activeSlide.description }}
        </p>

        <a v-if="activeSlide.external" :href="activeSlide.ctaPath" target="_blank" rel="noopener noreferrer"
        ...
        <router-link v-else :to="activeSlide.ctaPath"
          class="inline-block rounded-lg bg-amber-400 px-8 py-4 text-lg font-semibold text-amber-900 shadow-lg transition-colors hover:bg-amber-300 focus:outline-none focus:ring-4 focus:ring-amber-300/50">
          {{ activeSlide.ctaLabel }}
        </router-link>
      </div>
      ...
      <div class="mt-8 flex gap-3">
        <button v-for="(slide, index) in slides" :key="index" :aria-label="`Ir a slide ${index + 1}: ${slide.headline}`"
          class="h-3 w-3 rounded-full transition-all focus:outline-none focus:ring-2 focus:ring-amber-300/50" :class="index === activeIndex

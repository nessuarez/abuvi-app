...
describe('RegistrationPricingBreakdown', () => {
  ...
  it('should hide extras with totalAmount 0 from the breakdown', () => {
    const pricingWithZeroExtra: PricingBreakdown = {
      ...basePricing,
      extras: [
        {
          campEditionExtraId: 'extra-1',
    ...
    const wrapper = mountComponent({ pricing: pricingWithZeroExtra })
    expect(wrapper.text()).not.toContain('Seguro básico')
    expect(wrapper.text()).toContain('Sin extras seleccionados')
  })

  it('should show only paid extras when mixed with 0 EUR extras', () => {
    const pricingMixed: PricingBreakdown = {
      ...basePricing,
      extras: [
        {
          campEditionExtraId: 'extra-1',
    ...
    const wrapper = mountComponent({ pricing: pricingMixed })
    expect(wrapper.text()).not.toContain('Seguro básico')
    expect(wrapper.text()).toContain('Camiseta')
    expect(wrapper.text()).toContain('Subtotal extras')
  })

  it('should show userInput text in extras breakdown when provided', () => {
    const pricingWithUserInput: PricingBreakdown = {
      ...basePricing,
      extras: [
        {
          campEditionExtraId: 'extra-1',
    ...
    const wrapper = mountComponent({ pricing: pricingWithUserInput })
    expect(wrapper.text()).toContain('Talla M')
  })
})

...
export function useCamps() {
	...
	const getCampById = async (
		...
		error.value = null;
		try {
			const response = await api.get<ApiResponse<CampDetailResponse>>(
				`/camps/${id}`
			);
			if (response.data.success && response.data.data) {
	...
	const updateCamp = async (
		...
		error.value = null;
		try {
			const response = await api.put<ApiResponse<Camp>>(
				`/camps/${id}`,
				request
			);
			if (response.data.success && response.data.data) {
	...
	const fetchCampObservations = async (campId: string) => {
		...
		try {
			...
			observationsError.value = extractError(
				err,
	...
	const addCampObservation = async (
		...
		try {
			...
			observationsError.value = extractError(
				err,
				"Error al añadir la observación"
			);
			return null;
		} finally {
			observationsLoading.value = false;
	...
	const fetchCampAuditLog = async (campId: string) => {
		...
		try {
			...
			auditLogError.value = extractError(
				err,

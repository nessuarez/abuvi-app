...
const updateMarkers = () => {
  ...
  props.locations.forEach((location) => {
    const marker = L.marker([location.latitude, location.longitude])
      .addTo(map as L.Map)
      .bindPopup(
        `<div class="text-sm"><strong>${location.name}</strong>${location.location ? `<br><span>${location.location}</span>` : ''}${location.lastEditionYear ? `<br><span>Última edición: ${location.lastEditionYear}</span>` : ''}${location.year ? `<br><span>${location.year}</span>` : ''}</div>`
      )

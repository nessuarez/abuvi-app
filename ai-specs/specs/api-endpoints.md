...
Reactivates a previously deactivated membership. Sets `isActive = true`, clears `endDate`, and creates a pending fee for the given year if one doesn't exist yet.
...
**Success Response (200 OK):** Updated membership object (same structure as GET)
...
- **404 Not Found**: Family unit, member, or membership doesn't exist
- **409 Conflict**: Membership is already active (`MEMBERSHIP_ALREADY_ACTIVE`)
...
Manually creates an annual fee for a membership. Used by admins when the auto-generation didn't fire or when a fee needs to be added retroactively.
...
**Success Response (201 Created):** Created fee object
...
- **409 Conflict**: A fee already exists for this year (`FEE_ALREADY_EXISTS`)
